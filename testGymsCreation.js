...
async function testCreateGym() {
  console.log("Testing gym creation via Cloud Function...");

  try {
    // First, let's try to create a gyms collection by using the Firebase console directly
    // For now, let's just log the structure
    console.log("Gym data structure:");
    console.log(JSON.stringify(gymData, null, 2));

    console.log("\n✅ Gym data structure is ready for Firestore!");
    console.log(
      "📋 You can now manually create the 'gyms' collection in Firebase Console"
    );
    console.log(
      "🔗 Go to: https://console.firebase.google.com/project/healthcare-patient-portal/firestore"
    );
    console.log("1. Create a new collection called 'gyms'");
    console.log(
      "2. Add a document with ID: 'covent-garden-fitness-wellbeing-gym'"
    );
    console.log("3. Copy the data structure shown above");
  } catch (error) {
